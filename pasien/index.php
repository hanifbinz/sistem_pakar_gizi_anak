    ...
    if (mysqli_num_rows($check) > 0) {
        echo "<script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({ icon: 'warning', title: 'Maaf!', text: 'Data pasien sudah ada.', confirmButtonColor: '#007bff' });
            });
        </script>";
    } else {
        ...
        echo "<script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Data pasien berhasil disimpan.', showConfirmButton: false, timer: 1500 })
            ...
            <div class="col-12">
                <div class="card shadow-sm border-0">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="mb-0 text-primary">Daftar Akun Pasien</h4>
                                    ...
                                    ?>
                                            <tr>
                                                ...
                                                <td class='text-center'>
                                                    <a href='edit.php?id=<?= $row['id_pasien'] ?>' class='btn btn-primary btn-sm mr-1' title='Edit Data'><i class='fas fa-pencil-alt'></i></a>
                                                    <a href='delete.php?id=<?= $row['id_pasien'] ?>' class='btn btn-danger btn-sm delete-data' title='Hapus Data'><i class='fas fa-trash'></i></a>
                                                </td>
                                            </tr>
                                    <?php
                                        }
        ...
        <div class="modal-content border-0 shadow-lg">
            <form action="" method="POST">
                <div class="modal-header bg-light">
                    <h5 class="modal-title text-primary">Tambah Data Pasien</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="form-group"><label>Nama Lengkap</label><input type="text" class="form-control" name="namapasien" required></div>
                    ...
                    <div class="form-group"><label>Username Akun</label><input type="text" class="form-control" name="username" required></div>
                    <div class="form-group"><label>Password Akun</label><input type="password" class="form-control" name="password" required></div>
                </div>
